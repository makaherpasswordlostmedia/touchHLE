...
fn substitute_classes(
    ...
    // from the AdMob SDK (or other ads SDK). This is a third-party advertising SDK. Naturally it
    ...
    // devices anyway :)
    if !(name.starts_with("AdMob") || name.starts_with("AltAds") || name.starts_with("Mobclix")  || name.starts_with("OpenFeint")) {
        return None;
    }
