...
fn __strcpy_chk(env: &mut Environment, dest: MutPtr<u8>, src: ConstPtr<u8>, _size: GuestUSize) -> MutPtr<u8> {
    strcpy(env, dest, src)
}
...
fn strncat(
    ...
    GenericChar::<u8>::strncat(env, dest, src, size)
}
fn strncpy(
    env: &mut Environment,
    dest: MutPtr<u8>,
