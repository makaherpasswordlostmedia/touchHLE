...
///! AVAudioPlayer
///
/// Implemented using Audio Queue Service based on the PlayingAudio example
...
use crate::frameworks::audio_toolbox::audio_queue::{
    AudioQueueAllocateBuffer, AudioQueueBuffer, AudioQueueBufferRef, AudioQueueDispose,
    AudioQueueEnqueueBuffer, AudioQueueNewOutput, AudioQueueOutputCallback, AudioQueuePause,
    AudioQueueRef, AudioQueueStart, AudioQueueStop,
};
...
// Listing 3-7 from `Deriving a playback audio queue buffer size` from the Apple's guide
fn derive_buffer_size(
    ...
    let mut out_num_packets_to_read = 0;
    ...
    if out_buffer_size > max_buffer_size && out_buffer_size > max_packet_size {
        ...
        if out_buffer_size < min_buffer_size {
