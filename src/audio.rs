...
impl AudioFile {
    pub fn open_for_reading<P: AsRef<GuestPath>>(path: P, fs: &Fs) -> Result<Self, ()> {
        ...
        // a lot of changes in Audio Toolbox.
            ...
            if let Ok(pcm_ram) = pcm_loader.load_memory(Cursor::new(bytes), None, ResampleQuality::Linear, None) {
    ...
    pub fn audio_description(&self) -> AudioDescription {
        match self.0 {
            ...
            AudioFileInner::PcmRAM(ref pcm_ram) => AudioDescription {
                // TODO: fix this
                sample_rate: f64::from(pcm_ram.sample_rate() / 2),
                format: AudioFormat::LinearPcm {
                    is_float: true,
                    is_little_endian: true,
                },
    ...
    pub fn byte_count(&self) -> u64 {
        match self.0 {
            ...
            AudioFileInner::PcmRAM(ref pcm_ram) => {
                u64::from(pcm_ram.len_frames() as u32) * self.bytes_per_sample()
            }
        }
    }
    ...
    pub fn packet_count(&self) -> u64 {
        match self.0 {
            AudioFileInner::Wave(_) | AudioFileInner::Mp3(dr_mp3::Mp3DecodedToPcm { .. }) | AudioFileInner::PcmRAM(_) => {
                // never variable-size
        ...
        match self.0 {
            ...
            AudioFileInner::PcmRAM(_) => {
                ...
                for i in 0..sample_count {
                    if i % 2 == 0 {
                        let left_sample: f32 = samples[0][offset + i];
                        buffer[byte_offset..][..f_size].copy_from_slice(&left_sample.to_le_bytes());
                    } else {
                        let right_sample: f32 = samples[1][offset + i];
                        buffer[byte_offset..][..f_size].copy_from_slice(&right_sample.to_le_bytes());
                    }
                    byte_offset += f_size;
                }
                Ok(byte_offset)
